...
absolute right rule : 
    f(x) = 2*(Age%3-1) + (Attitude==positive) - (Attitude==negative) + 0.5*(Research_time-5) + (Entertainment==pokemon_go) + 
            + (Health-2)/5 - 2*(Changed_Major==1) - (Birthplace%7==1)/(Cost/200) + (ID==Q) - 2*(ID==K2)
    if f(x)>=0,lable is 1
...
def G_label(temp):
    f = 2*(temp[1]%3-1) + (temp[6]=='positive') - (temp[6]=='negative') + 0.5*(temp[9]-5) + (temp[10]=='pokemon_go') + (temp[14]-2)/5 - 2*(temp[13]==1) - (temp[5]%7==1)/(temp[11]/200) + (temp[0][0]=='Q') - 2*(temp[0][0:1]=='K2')
    if(f>=0):    return 1
    else: return 0
...
data_shuffled.to_csv('original_data_2.csv', index=False)

# train data and test data split
original_data = pd.read_csv('original_data_2.csv')
